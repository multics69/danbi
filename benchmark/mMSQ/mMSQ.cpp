/*                                                                 --*- C++ -*-
  ...
  mMSQ.cpp -- micro-benchmark for SSQPS
 */
#include <unistd.h>
#include <getopt.h>
#include <string.h>
...
class mMSQWorker: public Thread, private AbstractRunnable {
private:
...
public:
  mMSQWorker(volatile bool& Running_, BenchmarkParam* BP_, 
               MSQueue<Element>& Q_, Element* Elm_)
...
int benchmarkMain(BenchmarkParam *BP) {
  // Create a queue
  MSQueue<Element>* Q = new MSQueue<Element>(); 
  if (!Q)  return -ENOMEM; 
  ...
  std::list<mMSQWorker*> Workers; 
  for (int i = 0; i < BP->thread; ++i) {
    Element* Elm = new Element(); 
    if (!Elm) return -ENOMEM; 
    mMSQWorker* Worker = new mMSQWorker(Running, BP, *Q, Elm); 
  ...
  Running = false; 
  Machine::mb(); 
  for(std::list<mMSQWorker*>::iterator 
...
void showUsage(void) {
  ::fprintf(stderr, 
            "DANBI micro-benchmark for Michael-Scott Queue\n"); 
  ::fprintf(stderr, 
            "  Usage: mMSQRun \n"); 
  ::fprintf(stderr, 
            "      [--pinning {yes|no}]\n"); 
