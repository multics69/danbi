/*                                                                 --*- C++ -*-
  ...
  mLSQ.cpp -- micro-benchmark for SSQPS
 */
#include <unistd.h>
#include <getopt.h>
#include <string.h>
...
class mLSQWorker: public Thread, private AbstractRunnable {
private:
...
public:
  mLSQWorker(volatile bool& Running_, BenchmarkParam* BP_, 
               LSQueue<Element>& Q_, Element* Elm_)
...
int benchmarkMain(BenchmarkParam *BP) {
  // Create a queue
  LSQueue<Element>* Q = new LSQueue<Element>(); 
  if (!Q)  return -ENOMEM; 
  ...
  std::list<mLSQWorker*> Workers; 
  for (int i = 0; i < BP->thread; ++i) {
    Element* Elm = new Element(); 
    if (!Elm) return -ENOMEM; 
    mLSQWorker* Worker = new mLSQWorker(Running, BP, *Q, Elm); 
  ...
  Running = false; 
  Machine::mb(); 
  for(std::list<mLSQWorker*>::iterator 
...
void showUsage(void) {
  ::fprintf(stderr, 
            "DANBI micro-benchmark for Ladan-Mozes-Shavit Queue\n"); 
  ::fprintf(stderr, 
            "  Usage: mLSQRun \n"); 
  ::fprintf(stderr, 
            "      [--pinning {yes|no}]\n"); 
