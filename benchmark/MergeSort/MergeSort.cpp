/*                                                                 --*- C++ -*-
  ...
  MergeSort.cpp -- parallel mergesort
...
__kernel void Combine(void) {
  ...
  DBG_EXEC_COMBINE( printf("[0x%x] { Run Combine Cmd\n", Thread::getID()) ); 
...
#ifdef DBG_ACCOUNT_MERGE_ELM
  DBG_EXEC_COMBINE( printf("[0x%x] (%d) } Run Combine Cmd (%d, %d) q %d rmg: %d\n", 
...
int benchmarkMergeSort(int Cores, int Iter, float QFactor, int Elements, char *Log) {
  ...
  //                        +------------------------+
  // --[TestSink]
  ProgramDescriptor PD("MergeSort"); 
  ...
  PD.KernelTable["03CombineK"]->InQ[2] = "MergedDataQ";
  ...
  PD.KernelTable["05TestSinkK"] = new KernelDescriptor("TestSink", false, false); 
  PD.KernelTable["05TestSinkK"]->InQ[0] = "FinalMergedDataQ";
  PD.KernelTable["05TestSinkK"]->ROB[0] = "I";
  PD.KernelTable["05TestSinkK"]->ROB[1] = "N";
...
int parseOption(int argc, char *argv[], int *iterations, int *cores, char ** log) {
  ...
  *iterations = 100; 
  *cores = 1; 
  *log = const_cast<char *>("MergeSort-Log");
